...
(def filter-hierarchy
  "Experimental! For future use with multimethod dispatch, `is-clause?`, `match`/`replace`. For example, you could
  handle either `:and` or `:or` clauses with a single multimethod by using `::compound-filter` as a dispatch
  value. (Experimental -- if this proves useful, I may extend it out and use it for other clauses as well.)"
  (loop [h (make-hierarchy), [[child parent] & more] [[::equality-filter    ::filter]
                                                      [:=                   ::equality-filter]
                                                      [:!=                  ::equality-filter]
                                                      [:is-null             ::equality-filter]
                                                      [:not-null            ::equality-filter]
                                                      [::comparision-filter ::filter]
                                                      [:>                   ::comparision-filter]
                                                      [:<                   ::comparision-filter]
                                                      [:>=                  ::comparision-filter]
                                                      [:<=                  ::comparision-filter]
                                                      [:between             ::comparision-filter]
                                                      [::string-filter      ::filter]
                                                      [:contains            ::string-filter]
                                                      [:does-not-contain    ::string-filter]
                                                      [:starts-with         ::string-filter]
                                                      [:ends-with           ::string-filter]
    ...
    (if-not child
      h
      (recur (derive h child parent) more))))
